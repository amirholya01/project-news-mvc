...
class Auth
{
    ...
    {
        require_once BASE_PATH . '/app/view/auth/register.php';
    }
    ...
    public function registerStore($request)
    {
        if (empty($request['email']) || empty($request['username']) || empty($request['password'])) {
            ...
            $db = new User();
            $user = $db->findByEmail($request['email']);
            if ($user != null) {
    ...
    {
        $db = new User();
        $user = $db->findByToken($verifyToken);
        if ($user == null) {
            $this->redirect('login');
        } else {
            $result = $db->update($user['id'], ['is_active' => 1]);
            $this->redirect('login');
        }
    }
    ...
    public function checkAdmin()
    {
        if (isset($_SESSION['user'])) {
            $db = new User();
            $user = $db->findBySession($_SESSION['user']);
            if ($user != null) {
    ...
    {
        require_once BASE_PATH . '/app/view/auth/forgot.php';
    }
    ...
    public function forgotRequest($request)
    {
        if (empty($request['email'])) {
            ...
            if ($user == null) {
                ...
                if ($result) {
                    $db->update($user['id'], ['forgot_token' => $randomToken, 'forgot_token_expire' => date('Y-m-d H:i:s', strtotime('+15 minutes'))]);
                    $this->redirect('login');
                } else {
    ...
    {
        require_once BASE_PATH . '/app/view/auth/reset-password.php';
    ...
    public function resetPassword($request, $forgot_token)
    {
        if (!isset($request['password']) || strlen($request['password']) < 8) {
            ...
            $db = new User();
            $user = $db->findByForgotToken($forgot_token);
            if ($user == null) {
                ...
                if ($user) {
                    $db->update($user['id'], ['password' => $this->hash($request['password'])]);
                    $this->redirect('login');
                } else {
                    flash('reset_error', 'کاربر یافت نشد');
                    $this->redirectBack();
